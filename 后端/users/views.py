...
def get_userid(request):
    if request.method == 'GET':
        global userid
        user_id = userid
        return JsonResponse({'result': user_id, 'message': '成功'})
...
def login(request):
    if request.method == 'POST':
        ...
        if not User.objects.filter(username=username).exists():
            result = {'result': -1, 'message': r'用户不存在!'}
            return JsonResponse(result)
...
def upload_avatar(request):
    if request.method == 'POST':
        ...
        # 上传
        if upload_result == -1:
            result = {'result': 0, 'message': r"上传失败1！"}
        ...
        if upload_result == -1:
            os.remove(os.path.join(BASE_DIR, "media/" + avatar.name))
            result = {'result': 0, 'message': r"上传失败2！"}
            return JsonResponse(result)
        ...
        if not url:
            os.remove(os.path.join(BASE_DIR, "media/" + avatar.name))
            result = {'result': 0, 'message': r"上传失败3！"}
            return JsonResponse(result)
        # 获取对象存储的桶地址
        user.avatar_url = url
        user.save()
...
def getfollow(request):
    if request.method == 'POST':
        user_id = request.POST.get("user_id")
        users_id = get_follow_list_simple(user_id)
        user_list = []
        for i in users_id:
...
def getfan(request):
    if request.method == 'POST':
        user_id = request.POST.get("user_id")
        users_id = get_fan_list_simple(user_id)
        user_list = []
        for i in users_id:
            user = User.objects.get(id=i)
            a = user.to_simple_dic()
...
def havefollow(request):
    if request.method == 'GET':
        global userid
        user_id = userid
        follow_list = []
        for followid in get_follow_list_simple(user_id):
            follow_list.append(followid)
            result = {'result': 1, 'follow_list': follow_list}
        if follow_list == []:
            result = {'result': 0}
            return JsonResponse(result)
        else:
...
def follow(request):
    if request.method == 'POST':
        ...
        if follow_id == user_id:
            result = {'result': 2, 'message': r"不能关注自己!"}
            return JsonResponse(result)
        # 是否已关注

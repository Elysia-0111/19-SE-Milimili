        ...
        <div class="lower-left-options">
          <div style="display: flex;justify-content: flex-start;align-items: center;height: 70px;">
            ...
            <div class="lower-left-options-li lower-left-options-collection" @click="btnCollection">
              ...
              <span :class="{ 'btn-active': isCollected }">{{ video['collect_num'] }}</span>
            </div>
          </div>
          ...
          <ul>
            <li class="lower-comments-li" v-for="item in comments" :key="item.id">
              ...
              <div class="lower-comments-li-text">
                ...
                <!--<div class="lower-comments-li-text-time">{{ item.comment_time }}
                  <span class="reply_btn" @click="replyBtn(item.id, item.username, index)">回复</span>
                  <span class="lower-comments-li-text-more">展开</span>
                </div>-->
              </div>
              ...
              <div class="lower-reply reply-li" style="position: unset;border-bottom: none;">
                <div style="display:flex;justify-content:space-between;align-items: flex-start;">
                  ...
                  <div class="lower-reply-input">
                    <textarea autocomplete="false" rows="3" placeholder="" v-model="reply_list[index]"></textarea>
                  </div>
                </div>
      ...
      <!--<div class="lower-more" ref="lower_more">
        <div class="video-change">
          ...
          <div class="video-change-ul">
            <div v-for="(item, index) in videoList" :key="index" class="video-change-li" @click="changeVideo(item, index)">
              <span>
                {{ item.split(".")[0] }}
              </span>
            </div>
...
export default {
  ...
  methods: {
    ...
    replyForUser(userName, commentId, index) {
      ...
      this.addComment(commentId, text);
  ...
  mounted() {
    const player = new Dplayer({
      ...
      video: {
        url: 'https://milimili-file-1317998312.cos.ap-beijing.myqcloud.com/' + this.getVideoId() + '.mp4'
      },
      theme: "#9AC8E2",
    });
    this.player = player;
